...
with recursive cte as
  ...
  ,least(capacity,total_passengers) as total_onboarded_bus
  from cte_data
  where rn=1
      union all
	 select d.rn,d.bus_id,d.capacity,d.total_passengers
	 , least(d.capacity,d.total_passengers - cte.total_onboarded_bus) as onboarded_bus
	 , cte.total_onboarded_bus + least(d.capacity, d.total_passengers - cte.total_onboarded_bus) as onboarded_bus
	 from cte
